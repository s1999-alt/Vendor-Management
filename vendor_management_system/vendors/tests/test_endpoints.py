...
class VendorAPITestCase(APITestCase):
    def setUp(self):
        ...
        User.objects.create_superuser(username='superuser', email='superuser@example.com', password='superpassword')
    ...
    def test_retrieve_nonexistent_vendor_endpoint(self):
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.get_token())
        url = reverse('vendor-retrieve-update-destroy', kwargs={'pk': 999})  # Non-existent vendor ID
        response = self.client.get(url)
...
#     self.assertEqual(self.vendor.contact_details, "9633911996")
#     self.assertEqual(self.vendor.address, "123 Main St")
...
#       delivery_date=timezone.now().date() + timezone.timedelta(days=5),
...
#         vendor_code="TEST123"
#     )
#     self.historical_performance = HistoricalPerfomance.objects.create(
#         vendor=self.vendor,
...
#       self.assertEqual(self.historical_performance.quality_rating_avg, 4.5)
