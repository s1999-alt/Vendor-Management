...
class Vendor(models.Model):
  ...
  def clean(self):
     ...
     if not re.match(r'^\d{10}$', self.contact_details):
         raise ValidationError({'contact_details': 'Contact details must contain exactly 10 digits.'})
  ...
  def calculate_on_time_delivery_rate(self):
     ...
     if total_completed_pos == 0:
        return 0.0
     on_time_deliveries = completed_pos.filter(delivery_date__lte=models.F('issue_date') + timedelta(days=1)).count()
  ...
  def calculate_quality_rating_average(self):
     ...
     if completed_pos.exists():
        return completed_pos.aggregate(avg_quality_rating=Avg('quality_rating'))['avg_quality_rating'] or 0.0
     return 0.0
  
  def calculate_average_response_time(self):
     completed_pos = self.purchaseorder_set.filter(status='completed').exclude(acknowledgment_date__isnull=True)
     if completed_pos.exists():
        response_times = [(po.acknowledgment_date - po.issue_date).days for po in completed_pos]
...
class PurchaseOrder(models.Model):
  ...
  quality_rating = models.FloatField(blank=True, null=True, validators=[MinValueValidator(0.0), MaxValueValidator(5.0)])
  issue_date = models.DateTimeField(auto_now_add=True)
  acknowledgment_date = models.DateTimeField(blank=True, null=True)

  def clean(self):
     if self.order_date > self.delivery_date:
        raise ValidationError({'delivery_date': 'Delivery date must be after order date.'})
